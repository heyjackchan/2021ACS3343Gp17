...
public class GameController {
    ...
    public GameController(Dealer dealer, AbstractPlayer[] players, int currentPlayerIndex, int round, int turn,
            int skipCount, boolean isFirstPlayer) {
        this.dealer = dealer;
    ...
    public void printRemainingNumberOfCards() {
        for (int i = 0; i < 4; i++) {
            if (i != currentPlayerIndex) {
                System.out.println(
                        players[i].getName() + " still have " + players[i].getHand().getCardNo() + " card(s).");
            }
        }
    }
